...
@Component({
    ...
    template: `
      <div class="preset-switcher">
        <button class="text-button text-button--white-outline preset-switcher__main"
                (click)="socket.changePreset(preset.preset, !preset.state, preset.modifier)"
        ...
        <ul class="list">
          <ng-container *ngIf="'preset.config.select'">
            <li class="list__item" *ngFor="let select of preset.config.select">
              <button class="text-button button--white preset-switcher__sub"
                      [class.is-selected]="select.value === preset.modifier"
                      [class.is-active]="preset.state && select.value === preset.modifier"

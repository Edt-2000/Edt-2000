...
export const program$: Observable<IMidiProgramMsg> = getMidiObservable<IMidiProgramMsg>(MidiMsgTypes.program).pipe(
    map(program => ({
        ...program,
        channel: program.channel + 1,
    })),
);
export const select$: Observable<IMidiSongMsg> = getMidiObservable<IMidiSongMsg>(MidiMsgTypes.select).pipe(
    map(select => ({
        ...select,
        channel: select.channel + 1,
    })),
);
export const clock$: Observable<void> = getMidiObservable<void>(
    MidiMsgTypes.clock,
);
export const BPM$: Observable<void> = clock$.pipe(
    bufferCount(24),
    map(() => {
    }),
);
export const CC$: Observable<IMidiCCMsg> = getMidiObservable<IMidiCCMsg>(
    MidiMsgTypes.cc,
).pipe(
    map(cc => ({
        ...cc,

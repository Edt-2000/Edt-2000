...
import {
    automationChannel,
    imageAutomationCCNumber,
    videoAutomationCCNumber,
    wordAutomationCCNumber,
} from "../../config/config";
...
export const imageSrcActions$ = midiCCAutomation$.pipe(
    ...
    map(([{ value: fileName }, { title, songNr }]) => {
        return Actions.imageSrc(
            songNr.toString().padStart(3, "0") +
                "_" +
                title +
                "/" +
                fileName.toString().padStart(3, "0") +
                ".jpg",
        );
    }),
);
